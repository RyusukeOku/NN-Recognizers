...
# ★ ModelInputデータクラスにinput_idsフィールドを追加
...
class RecognitionModelInterface(ModelInterface):
    ...
    def construct_model(self,
        ...
        **kwargs 
        ...
        # コマンドライン引数の代わりに、ここで値を変更してください
        add_ngram_head_n = 2

        if architecture is None:
            raise ValueError
        
        # この変数にモデルの主要部分を格納していきます
        core_pipeline = None 
        output_size = 0
        shared_embeddings = None

        if architecture == 'transformer':
            if num_layers is None: raise ValueError
            if d_model is None: raise ValueError
            if num_heads is None: raise ValueError
            if feedforward_size is None: raise ValueError
            if dropout is None: raise ValueError
            ...
            if hidden_units is None: raise ValueError
            if num_layers is None: raise ValueError
            if dropout is None: raise ValueError
            ...
            if architecture == 'rnn':
                core = SimpleRNN(
                    input_size=hidden_units, hidden_units=hidden_units, layers=num_layers,
                    dropout=dropout, learned_hidden_state=True
                )
            else:
                core = LSTM(
                    input_size=hidden_units, hidden_units=hidden_units, layers=num_layers,
        ...
        # n-gramヘッドの挿入ロジック
        # core_pipeline に @ 演算子で追加していきます
        if add_ngram_head_n > 0:
            ngram_head_layer = NgramHead(
                n=add_ngram_head_n,
                d_model=output_size
            )
    ...
    def get_logits(self, model, model_input):
        ...
        if self.add_ngram_head_n > 0:
            core_internal_tag_kwargs['ngram_head'] = {
